...
| 3 | `Device Setting` F83 | D83을 넘으면 OVP2에서 VCC가 25 V를 넘는다 — **보조권선을 VCC 에 바로 물릴 때만.** 본 설계는 2026-09-25 부터 제너 레귤레이터를 거치므로 F83 1.0 > D83 0.84 가 **의도된 차이**다(B84 · CHANGELOG, 시트 §14c `k.VCC`). **정수 턴 구현 가능성도 확인** |
