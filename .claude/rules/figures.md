...
- 코어 기하는 `cores.py` 한 곳(`CORES` 전자기 값 · `MECH` 치수 · `PINMAP` 핀 배정 · `winding()`), 그림·표·벤더 사양서가 같이 읽는다.
  `J_CU` · `K_U` · `K_LITZ` · `TIW_LITZ_ADD` · `PRI_SPLIT` · `SEC_PAR` · `G_NOM` · `MARGIN_F` · `TIW_OD` 가 가정·선정값이다. 절연은 TIW 선재가 맡고 간격은 누설 전용 — 간격을 바꾸면 `leakage.py` 로 L_short 를 다시 볼 것. **핀 번호 방향은 가정**이라 도면으로 확인할 것.
