...
**트랜스포머 절연은 미·EU·일·한·중 가정용 AV 기준 강화절연**(`insulation.py`, `docs/DESIGN.md` §4.4) — 2026-09-25. **AN 예제 트랜스포머는 TDK PC47EE60-Z(E 60/22/16) · 주문 보빈 섹션 권선**(온세미 방식 — 1차 TIW-Litz 1가닥 4층(4·4·4·3 턴) 한 칸 · 2차 턴당 Litz 3다발 나란히, 층당 1턴 4층(튜브에서 NS2·NS3·NS2·NS3) 한 칸 · **두 칸 맞닿음(칸막이 없음)** · 층마다 테이프, 절연은 전선, 권선 피치 k_w 1.10 가정 — 69차). `leakage.py` 추정 **11.46 µH**(목표 11, +4.2 %) — 칸막이가 없어 첫 샘플 뒤 맞출 손잡이가 없다. 남은 위험: **반쪽 누설 NS2 12.26 · NS3 11.61 µH**(높은 쪽이 L_r +11 %), 중앙다리 단일 갭 약 3.8 mm(TDK 곡선 외삽), Litz 와전류 손실(`docs/DESIGN.md` §4.2 13). 남은 것은 문서 작업과 실측 뒤의 `R.T` 결정이다(아래 "다음 할 일").
