# L6790A 단일단 PF LLC — 25 V / 26.3 A TV(OLED) SMPS — L_r 14 µH 복사본

> **이 폴더(`Lr14/`)는 탱크를 L_r 14 µH · L_m 25.46 µH · C_r 100 nF 로 바꾼 설계의 복사본이다**(2026-09-26 사용자: "이거 용으로 따로 복사해서 만들어 기존꺼 수정하지 말고"). 원본(저장소 최상위, L_r 11 · L_m 20 µH)은 건드리지 않는다. λ = 0.55 그대로라 권선비 n 은 같고 f_r 이 151.8 → 134.5 kHz 로 내려간다. 예제 트랜스포머가 창을 **빈틈없이** 채우도록(칸막이가 남는 폭 전부) L_r 을 맞춘 값이다. `Reference/` 는 원본 폴더로의 심볼릭 링크. 도구는 이 폴더 안에서 돌린다(경로는 스크립트 기준 상대경로라 그대로 동작). Stop hook(`guard.py`)은 원본만 검사하므로 이 폴더의 검사는 손으로 돌린다.
...
설계 수치 — 시트 · 워크북(입력 셀) · `l6790.py` 세 곳이 일치. 판정 지표 44개 중 **정본(코어 3개) 40개**(원본보다 `k.Ae` 하나 더 미달 — f_r 이 내려가 자리표시자 EE6405 의 B_pk 가 205 mT, 필요 A_e 116 mm²) · **AN 예제(트랜스포머 1개) 41개** 통과
(미달 사유는 `docs/DESIGN.md` §3). **VCC 는 보조권선 2T + 제너 레귤레이터**(외부 VCC 없음, 워크북과 의도된 차이),
**트랜스포머 절연은 미·EU·일·한·중 가정용 AV 기준 강화절연**(`insulation.py`, `docs/DESIGN.md` §4.4) — 2026-09-25. **AN 예제 트랜스포머는 TDK PC47EE60-Z(E 60/22/16) · 주문 2칸 보빈 섹션 권선**(1차 TIW-Litz 1가닥 4층(4·4·4·3 턴) 한 칸 · **칸막이 1.73 mm**(플랜지 최소 1.35 mm, 남는 폭 전부) · 2차 턴당 Litz 3다발 나란히, 층당 1턴 4층(NS2·NS3·NS2·NS3) 한 칸 · 층마다 테이프, 절연은 전선, k_w 1.10 가정). `leakage.py` 추정 **13.99 µH**(목표 14), 반쪽 14.83 · 14.12 µH(±10 % 안). 칸막이는 **얇게만** 조정 가능(0.1 mm당 −0.15 µH). 남은 위험: 중앙다리 단일 갭 약 2.8 mm(TDK 곡선 외삽), Litz 와전류 손실, 2차 3다발 순환 전류 약 3.5 A(`docs/DESIGN.md` §4.2 13). 남은 것은 문서 작업과 실측 뒤의 `R.T` 결정이다(아래 "다음 할 일").
...
| `Calculation Excel Sheet/L6790_spreadsheet_r1_0_corrected_rev0_6.xlsx` | ST 툴 수정본, 7.5:1(`variants/…_7p5to1.xlsx` 와 같은 파일). `CHANGELOG_rev0_6` 에 변경 이력. **이 복사본은 `Res. Tank Design` F44 · F45 를 14 · 25.46 으로(`lr14_xl.py`) — 의존 수식의 캐시는 Excel 에서 한 번 열었다 저장해야 맞는다** | rev 0.6 + L_r 14 |
...
| **`R.T` — 11 kΩ 유지** | 원본에서는 트랜스포머 공차(±10 %)에 11 kΩ 이 부족했다. **이 복사본은 `k.floor` 1.02 → 1.15, `k.RTd` 1.02 → 1.16** 으로 여유가 커졌다 — 실측 후 결정은 같다 | 시제품 |
