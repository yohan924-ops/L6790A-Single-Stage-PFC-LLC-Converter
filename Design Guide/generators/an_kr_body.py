...
def build(A):
    ...
    add(p('대가도 있다. 주파수가 입력과 부하에 따라 움직이고, 자화 전류가 '
          '부하와 무관하게 순환하므로 Light load 효율은 보통이며, 설계가 듀티 '
          '식이 아니라 게인 곡선에서 나온다.'))
    add(h2('공진으로 무엇을 얻나'))
    ...
    add(p('그 다이오드는 저장 전하가 다 빠져나갈 때까지 막지 못하므로, 그동안 '
          ...
          '소자에서 손실로 빠진다. 다이오드가 끊길 때는 루프 인덕턴스의 di/dt 가 '
          '소자 양단에 오버슈트로 나타난다.'))
    ...
    add(p('capacitive 경계는 f<sub>o</sub> 와 f<sub>r</sub> 사이에 있고 '
          '<b>부하와 함께 올라간다</b>: No load 에서는 f<sub>o</sub>, '
          'Full load 에서는 f<sub>r</sub> 쪽으로 올라가 있다. Light load 에서 '
          'inductive 이던 주파수가 Overload 에서는 capacitive 일 수 있다.'))
    ...
    add(note('<b>설계 규칙.</b> f<sub>Min</sub> 을 f<sub>o</sub> 위에 둔다. '
             '그것은 No load 경계일 뿐이라 필요조건이지 충분조건이 아니다. 진짜 '
    ...
    add(fig('an_pfc_boost',
            'boost 보정기: 스위치가 켜진 동안 인덕터가 라인에서 충전되고, 꺼진 '
            '동안 라인과 직렬로 버스에 전달한다.'))
    ext(bullets([
    ...
    add(note('<b>맞바꾸는 것.</b> 나가는 것: 스위치, 인덕터, 다이오드, '
             ...
             '하는 탱크. 좋은 trade-off 인지는 부하가 출력 리플을 얼마나 받아 '
             '주느냐에 달렸다.'))
    ...
    add(p('그리고 No load LLC 는 f<sub>o</sub> 에서 무한한 게인을 갖는다. 두 '
    ...
    add(p('벌크 커패시터가 없으므로 탱크는 정류된 상용전원을 그대로 본다. 역률 1 '
          ...
          '답한다. 전류 루프도, 정류 기준파에 대한 multiplier 도 없다: <b>주파수 '
    ...
    add(p('마지막 두 행에서 게인 요구는 발산하지만, 그것을 맞출 능력도 같이 '
          '발산한다: No load 탱크는 f<sub>o</sub> 에서 무한한 게인을 갖는다. '
          '주파수는 f<sub>o</sub> 쪽으로 내려가고 전류는 전압과 함께 0 으로 '
          '간다.'))
    add(p('컨트롤러에는 시간 척도가 다른 신호가 둘 있다.'))
    ext(bullets([
        '<b>FB 는 양을 정한다.</b> error amp 출력을 옵토커플러로 '
        '받아 입력 <i>전력</i>을 지령한다(%s 절). 그 뒤의 루프는 라인 '
        '주파수보다 훨씬 아래에서 교차하므로 한 반주기 안에서는 상수다: 패널 '
        '4 프로파일의 <i>높이</i>.' % SR('피드백 핀은 전력 지령이다'),
        '<b>HVSU 는 주기의 어느 위치인지를 알린다.</b> 기동 핀이 입력 전압 sensing 도 '
    ...
    add(p('버스트 모드 식과 블록도의 multiplier 를 합치면 피드백 전압의 뜻이 '
          '나온다.'))
    ...
    add(p('<b>요구 최소 게인이 M<sub>&infin;</sub> 아래이면 어느 주파수도 '
          ...
          '것이다. 탱크가 점근선 위인지 아래인지는 1 %% 차이이고, Full load '
          '검사로는 보이지 않는다. %s 절이 이 설계의 결과를 보고한다.'
          % SR('숫자를 끝까지 따라가기')))
    add(note('해를 잃는 것이 실패는 아니다: 높은 입력 전압의 No load 는 '
             '<b>버스트 모드</b>의 몫이다. 하지만 어느 후보 권선비가 No load 에서 '
             '주파수만으로 조절할 수 있는지를 이것이 가르고, Full load 숫자로는 '
             '보이지 않는다.'))

    # =============================================================== 4
    add(h1('Topology morphing'))
    add(p('공진 탱크의 쓸 만한 게인 범위는 Full load 에서 대략 1.5:1 이다. '
    ...
    add(h2('컨트롤러는 어떻게 하나'))
    add(p('모드 사이에 바뀌는 게이트 신호는 하나뿐이다. threshold 는 <b>IC 안에 '
          '고정</b>돼 있다. R<sub>CFG</sub> 는 모핑을 켜고 브라운아웃 threshold 를 '
          '정할 뿐이다.'))
    ...
    add(note('<b>전환 과도.</b> threshold 를 넘으면 한 라인 주기 안에 구동이 2:1 로 '
    ...
    add(p('정격은 최악의 스위칭 사이클에서, 손실은 라인 사이클 실효값에서 '
          '온다. 규칙 둘이 더 있다.'))
    ext(bullets([
        '1차 소자와 sense 저항은 <b>합성</b> 탱크 전류를 본다: 반사 부하 전류 '
    ...
    ext(tbl('그림의 각 표시가 무엇을 정하나. 전류는 유닛 하나의 값이다: '
            ...
            [['표시', '항목', '어디서 &mdash; 무엇에 걸쳐 읽나',
              '무엇을 정하나'],
             ['<b>1</b>', '1차 피크 전류',
              'i<sub>p</sub> 의 피크: 자화 램프 위에 얹힌 부하 전류의 '
    ...
    add(p('코어는 독립된 조건 둘을 만족해야 한다. 정한 턴수에서 자속을 담을 '
          ...
          '곱이 <b>area product</b> 이고, 어느 쪽이든 걸릴 수 있다.'))
    ...
    add(h2('skin depth, 그리고 도선이 한 가닥이 아닌 이유'))
    ...
    add(p('&rho; 는 권선 온도에서의 구리 비저항, f 는 그 도선이 흘리는 '
          ...
          '못한다. 다른 턴의 자계도 각 도체에 순환 전류를 만드는데(proximity effect), '
    ...
    add(p('A<sub>cu</sub> 는 식&nbsp;%s 에서 온다. 다발은 동선보다 굵다. 둥근 '
          '가닥 사이의 틈, 에나멜, 서빙을 fill factor k<sub>litz</sub> 약 0.5 ~ '
          '0.6 으로 묶으면 외경은' % ER('window')))
    add(eq(r'd_{litz}=\sqrt{\frac{4A_{cu}}{\pi k_{litz}}}', key='litz'))
    add(p('<b>포일.</b> 두께는 skin depth 와 벤더 재고에서 고르고, 폭이 따라 '
    ...
    add(eq(r'w_{f}=\frac{A_{cu}}{t_{f}}\,,\qquad '
           ...
           key='foil'))
    add(note('이 식들은 전류 밀도와 skin depth 에 대해 동선을 정한다. skin effect 에 '
             'proximity effect 를 더한 AC 저항은 별개의 문제이고, 나란히 감은 권선에서는 '
             '보통 proximity effect 쪽이 크다.'))

    add(h2('절연, 마진, 그리고 창에서 치르는 값'))
    add(p('트랜스포머가 절연 장벽이다. <b>마진 테이프</b>는 보빈 양 끝에 creepage '
    ...
    add(note('<b>Open 인덕턴스 공차는 흔한 어림수로 둘 수 없다.</b> '
    ...
    add(note('컨트롤러의 기동 창은 유한하고, 이 뱅크는 보통 설계보다 훨씬 무거운 '
             '기동 부하다. 뱅크 전체로의 cold start 를 일찍 재 볼 것.'))
    ...
    add(p('%s 절이 부품을 순서대로 정한다. 가장 신경 쓸 것은 R<sub>T</sub> '
          ...
          '한다.' % SR('컨트롤러 주변 부품')))
    add(note('<b>그 하한이 보통 가장 작은 여유다.</b> 오실레이터 idle 시간에도 '
    ...
    add(h2('전압 루프와 보상'))
    add(p('루프는 그림&nbsp;%(f)s 처럼 이어진다: 분압기가 출력을 재고, TL431 이 '
    ...
    ext(tbl('루프가 이뤄야 할 것.',
            [['항목', '목표', '너무 낮으면', '너무 높으면'],
             ...
             ['2f<sub>l</sub> 에서의 G<sub>EA</sub>',
              '3차 고조파를 budget 안에 두는 값 이하(식&nbsp;%s).' % ER('GEAreq'),
              '루프에는 문제없다. crossover 주파수가 필요 이상으로 낮을 뿐이다.',
              '3차 고조파가 budget 을 넘고, 버스트 채터링.']],
    ...
    add(p('보상기가 그 중 G<sub>EA</sub>(2f<sub>l</sub>) 만큼을 FB 핀으로 '
          ...
          '생긴다. 지령을 m 만큼 변조하면 m/2 가 들어가고, 리플 진폭은 '
          '피크-피크의 절반이므로'))
    add(eq(r'D_{3}=\frac{G_{EA}(2f_{l})\,\Delta V_{loop}}{4\,V_{FB}}',
           key='D3'))
    add(p('뒤집으면, 3차 고조파 budget 이 2f<sub>l</sub> 에서의 최대 보상기 '
          '게인을 정한다.'))
    ...
    add(fig('comp_network_st',
            ...
            '정하고, TL431 이 error amp 이며, R<sub>f</sub>, C<sub>f</sub>, '
    ...
    add(p('전체에서 &omega; = 2&pi;f 이고, C<sub>ser</sub> 는 C<sub>F</sub> 와 '
          ...
          'pole(정상 오차 0)을 만들고, R<sub>F</sub>C<sub>F</sub> 가 위상을 '
          '되돌리는 zero f<sub>z</sub> 를, R<sub>F</sub> 와 C<sub>Fo</sub> 가 '
          '2f<sub>l</sub> 까지 게인을 다시 내리는 pole f<sub>p</sub> 를, '
          'C<sub>opto</sub> + C<sub>fx</sub> 와 R<sub>FB</sub> 가 스위칭 '
          '노이즈를 없애는 kHz 근처의 셋째 pole 을 만든다. CTR 이 게인 전체에 '
          '곱해지므로 그 산포가 중요하다.'))
    ...
    add(note('EA<sub>o</sub> 는 정상 상태 LED 전류에서의 CTR, CTR<sub>s</sub> '
             ...
             '빈의 위쪽 끝에서 루프를 다시 검사할 것: crossover 주파수는 '
    ...
    add(p('Venable 의 K-factor 법은 목표 둘, 곧 phase margin &Phi;<sub>M</sub> '
          ...
          'pole 을 정한다. phase margin 에서 퍼짐 계수 K<sub>v</sub> 가 나오고, '
    ...
    add(p('그림&nbsp;%(f)s 에 세 숫자를 어디서 읽는지 보였다. 루프 자신의 '
          'crossover 주파수 단위이므로 이런 루프 전부의 모양이다. %(s)s 절이 이 '
          '설계의 숫자를 같은 그림에 올린다.'
          % dict(f=FR('an_loop_example'), s=SR('전압 루프, 만든 대로'))))
    add(fig('an_loop_example',
            'Type II 보상기를 단 2적분기 루프, crossover 주파수 단위. phase margin 은 '
    ...
    add(p('f<sub>px</sub> 위에서는 zero 하나가 pole 둘을 상대하므로 위상이 '
    ...
    add(p('zero 가 두 pole 보다 충분히 아래에 있으면 근은 실수이고, K-factor '
          ...
          'crossover 주파수가 수십 Hz 이면 여유는 보통 크다. 그것은 계산할 '
          '이유이지 가정할 이유가 아니다.'))
    ...
    add(note('k<sub>Ploss</sub> = %(kPloss).3f 은 계산 실패가 아니라 결과다: '
             '고정된 1차 소자가 %(b).0f&nbsp;W budget 에 %(a).2f&nbsp;W 를 쓴다. '
             '고정 소자 자리에서 그 budget 을 만족하는 단일 600&nbsp;V 소자는 '
             '없으므로 설계는 그대로 가고, 히트싱크 문제는 실측으로 정한다'
    ...
    add(p('<b>STEP 1 &mdash; 탱크가 통과시키는 전력.</b> 브리지, EMI 필터, '
          'LLC 단이 각각 손실 budget 을 받고, 마지막은 &eta;<sub>HB</sub> 로 '
          '준다.'))
    ...
    add(fig('an_gain_design',
            ...
            '하프브리지의 230&nbsp;Vac 근처에 오므로, 두 입력 전압대가 '
    ...
    ext(bullets([
        ...
        '차이다: 모핑이 두 전압대를 탱크에 거의 같게 만든다. 264&nbsp;Vac(264)는 '
        ...
        '이다. 요구가 하한보다 <b>낮으므로</b> 높은 입력 전압의 No load 에서는 '
    ...
    ext(tbl('권선-핀 배정.',
            [['권선', '핀 (시작 &ndash; 끝)', '턴수', '도체', '줄'],
             ...
             ['NAUX 보조 (ZCD)', _CORE.pins('NAUX', '&ndash;'),
              '%d T' % V['Naux'], '아무 선, sensing 전용', _kr_row(_B['rows'][0])],
    ...
    ext(tbl('가정한 budget 과 계산한 소자 손실. 둘째 묶음은 첫째의 내역이 아니다.',
            [['항목', '값', '비고'],
    ...
    add(fig('an_loop_bode',
            '만든 루프의 |T| 와 180&deg; + arg&nbsp;T. 둘째 곡선은 crossover 주파수에서만 phase margin 이고, gain margin 은 f<sub>180</sub> 에서 '
            '읽는다.'))
    ext(tbl('전압 루프, 만든 대로.',
            [['항목', '값', '기준'],
    ...
    add(h2('전류 sense 저항, 한 번에 셋을 정한다'))
    ...
    add(p('유효 범위는 15 ~ 140&nbsp;k&Omega; 이고, 접지에 묶으면 버스트 모드가 '
          '꺼진다. 이 threshold 를 피드백 리플에 대해 검사할 것(%s 절). 아니면 '
    ...
    add(p('분압비만으로 두 과전압 threshold 가 정해진다. 절대값은 바이어스 전류만 '
          '정한다.'))
    ...
    add(calc(r'R_{ZCD,H}=%(RZL).0f\left(\frac{%(naux).1f\cdot%(t).1f}'
             ...
             % dict(V, naux=A.SH['n.aux'], t=A.SH['V.OVP1_out'],
                    rh=A.SH['R.ZCD_H'])))
    add(p('위 저항은 계산값 <i>바로 위의 E24 값</i>이라 OVP1 이 리플 피크를 '
          '비켜 간다. 실장한 쌍이 주는 값은'))
    ...
    add(h2('보호, 그리고 threshold 마다 어디서 정해지나'))
    add(p('threshold 마다 이미 고른 저항이 고정하므로, 한 이유로 바꾼 값이 다른 '
          '이유로 정한 threshold 를 움직인다.'))
    ...
    ext(bullets([
        ...
        '정해져 있지 않다.',
        '<b>계산값과 선정값을 따로 기록하고</b>, 뒤 단계의 모든 검사가 선정값을 '
        '읽게 한다.']))
    ...
    ext(bullets([
        ...
        '<b>%(Cout).1f&nbsp;mF 뱅크 전체로의 cold start.</b> 기동 창은 '
        ...
        '고정 소자의 손실이 가장 큰 곳이다. 받아들인 손실 budget 부족을 '
        '이것이 판정한다.',
    ...
    _SYM = [
        ...
        ('L<sub>p</sub>', 'L<sub>r</sub> + L<sub>m</sub>, No load 에서 탱크가 보이는 인덕턴스'),
        ...
        ('V<sub>ac,eq</sub>, V<sub>eq</sub>, V<sub>ac,eq,low</sub>, V<sub>ac,eq,high</sub>', '모핑 뒤 탱크가 보는 등가 입력 전압, 그리고 두 모핑 threshold 에서의 값'),
        ...
        ('I<sub>eq</sub>, I<sub>sat</sub>', '동작 자속을 재현하는 Open 전류; DC 중첩 시험 전류'),
        ...
        ('k<sub>Ploss</sub>', '소자 자리의 손실 budget 을 계산한 손실로 나눈 것'),
    ...
    ext(tbl('그럴듯하게 틀린 숫자를 만드는 함정.',
            [['어디서', '함정', '놓치면'],
             ...
             ['선정값 대 계산값',
              ...
              '뒤 단계의 모든 여유가 만들지 않은 설계에 대해 보고된다']],
    ...
    ext(bullets([
        ...
        '목표를 비례시킬 뿐이고, 뒤 단계에 % 수준으로 민감한 것은 없다.',
        '최대 전력 상수 <b>16.8 &Omega;&middot;W</b> 는 데이터시트의 피드백 폭과 '
        'multiplier 게인과 맞아떨어진다 &mdash; 2.8&nbsp;V / 0.167 = 16.8 &mdash; '
    ...
    ext(tbl('정해지지 않은 것, 그리고 무엇이 정하나.',
            [['항목', '상태', '무엇이 닫나'],
             ...
             ['1차 도통 손실',
              ...
              '고정 소자 자리에서 3 W budget 을 만족하는 단일 600 V 소자는 없다. '
              '히트싱크가 필요한지는 계산이 아니라 열 측정'],
             ['2차 손실 budget',
              '%(kPSR).3f &mdash; 사실상 다 썼다' % V,
             ...
             ['효율 가정',
              ...
              '움직일 뿐 뒤 단계에 민감한 것은 없다 &mdash; 그래도 실측으로 바꿔야 '
              '한다']],
