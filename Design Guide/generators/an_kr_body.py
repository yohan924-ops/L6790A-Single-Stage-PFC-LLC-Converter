...
def build(A):
    ...
    add(p('유니버설 입력 LLC 전원은 보통 두 단이다. 400&nbsp;V 버스를 만드는 '
          ...
          '들어오는 전력이 고르게 오지 않는 만큼을 저장하고, LLC 에는 거의 일정한 전압을 공급한다. 그래서 탱크는 좁은 게인 범위만 덮으면 '
          '된다.'))
    ...
    add(fig('an_fha_steps',
            '세 단계의 축약: 실제 회로, 2차를 1차로 넘긴 것, 기본파만 남긴 '
            '것. 결과는 게인 M 과 품질계수 Q 를 가진 AC 회로 하나다.'))
    ext(bullets([
    ...
    ext(bullets([
        ...
        '<b>i<sub>Lr</sub> 이 i<sub>Lm</sub> 과 같아진다.</b> 그때부터 2차로 '
        '넘어가는 전력이 없다: 프리휠링.',
    ...
    add(p('<b>어느 쪽에서 도는지는 파형 없이 차트만으로 읽을 수 있다.</b> '
          '요구 게인을 수평선으로 긋고, 현재 부하의 곡선과 만나는 점을 찾는다. '
    ...
    add(p('따라서 어느 쪽인지는 부하가 아니라 입력 전압이 정한다. 요구 게인은 '
          ...
          '교차점은 오른쪽으로 간다. 부하가 관여하는 것은 다른 질문이다: 교차점이 아직 피크의 inductive 쪽에 있는가.'))
    ...
    ext(bullets([
        ...
        '낮은 임피던스로 강제 역회복된다. <b>하드 스위칭이고, 부품을 파손시키는 고장이다.</b> capacitive 영역은 설계가 한쪽에만 머물러야 하는 '
    ...
    add(p('레그 하나를 보자. 하이사이드 스위치, 로우사이드 스위치, 중점에 '
          ...
          '데드타임 동안 어느 소자가 그 전류를 흘리느냐가 전부를 정한다. '
    ...
    ext(bullets([
        ...
        '다이오드를 통해 중점을 왔던 레일에 붙들어 둔다. 켜질 소자는 레일 '
    ...
    add(fig('an_loadshift',
            '경계는 부하와 함께 올라간다. 점선은 Q 를 바꿔 가며 그린 경계 자체다.'))
    ext(bullets([
    ...
    add(p('ZVS 는 전하 문제다. 데드타임 동안 자화 전류가 브리지 노드를 반대쪽 '
          '레일까지 흔들 만큼의 전하를 옮겨야 한다. 설계 변수는 브리지가 바뀐 뒤 '
    ...
    add(p('플라이백 설계 습관은 여기서 맞지 않는다. 그중 둘은 잘못하면 비용이 든다. <b>보내는 '
    ...
    add(fig('an_flyback_llc',
            ...
            '합, 색칠한 띠는 2차가 덜어 낸 것.'))
    add(p('<b>순서대로 만들어 보면.</b> 그림&nbsp;%s 은 두 컨버터의 한 주기를 '
          '구간별로 따라간다: 어느 권선이 도통하고, 그 권선에 어떤 전압이 '
          '걸리며, 그래서 자속이 어느 쪽으로 움직이고 무엇이 그것을 멈추는가.'
          % FR('an_flux_steps')))
    add(fig('an_flux_steps',
            '자속을 단계별로 만든 것. 왼쪽은 불연속 도통의 플라이백, 오른쪽은 '
            'below 의 LLC 로, 8구간 모델에 이 설계의 전류를 넣어 그렸다. 맨 '
            '아래 행이 핵심이다: 플라이백의 자속은 권선 전류를 따라가고 '
            '스위치가 꺼지는 곳에서 피크가 되지만, LLC 의 자속은 '
            'V<sub>out</sub>/(N<sub>s</sub>A<sub>e</sub>) 의 기울기로 정확히 '
            'T<sub>r</sub>/2 동안 오르다가 정류기가 끊기는 순간 멈춘다.'))
    ext(bullets([
        ...
        'V<sub>in</sub>/(N<sub>p</sub>A<sub>e</sub>) 로 오른다. 에너지는 갭에 '
        '쌓인다.',
        '<b>플라이백 2.</b> 스위치가 꺼진다. 자속은 뛸 수 없으므로 암페어턴이 '
    ...
    ext(tbl('같은 코어 둘, 나란히 비교.',
            [['', '플라이백', 'LLC'],
             ...
             ['갭이 하는 일',
              ...
              '정한다',
              'L<sub>m</sub>, 곧 자화 전류와 ZVS 를 정한다; 자화 에너지만 '
              '담는다'],
    ...
    ext(bullets([
        ...
        '아니다.',
        '<b>자속은 출력 전압과 함께 오른다.</b> 설계가 고려할 상한은 공칭 '
    ...
    ext(bullets([
        ...
        '확인한다.',
        '<b>전류 제한은 코어를 지키지 못한다.</b> OCP 는 동선과 반도체를 '
        '지킨다. 자속을 제한하는 것은 출력 OVP 이므로 포화 사양은 OVP '
        'threshold 에서 나온다(%s 절).'
        ...
        '<b>가장 비싼 플라이백 습관</b>은 탱크 피크 전류에서 포화하지 말라고 '
        '벤더에 요구하는 것이다. 컨버터가 만들 수 없는 자속을 위해 필요한 '
        '것의 두 배가 훨씬 넘는 코어를 요구하는 셈이다.']))
    add(p('DC overlap 시험은 2차를 열어 놓고 하므로 상쇄가 없다: 시험 전류 전부가 '
    ...
    add(p('P 는 평균 전력, I<sub>1,rms</sub> 는 기본파만의 실효값, '
          ...
          '(위상)는 모터 부하에서 나빠지고, <b>distortion</b>(실효값 중 고조파의 몫)은 정류기 부하에서 나빠진다. 위의 전류는 위상은 거의 맞는데도 '
          '0.6 정도밖에 안 된다.'))
    ...
    add(fig('an_pfc_boost',
            'boost PFC: 스위치가 켜진 동안 인덕터가 라인에서 충전되고, 꺼진 '
            '동안 라인과 직렬로 버스에 전달한다.'))
    ext(bullets([
        ...
        '느리게</b> 만든다. 빠른 전압 루프는 버스 리플에 반응해 PFC 가 만들려는 '
        '전류 파형을 망가뜨린다.',
    ...
    add(p('앞 절의 순서 중 두 단계는 여기서 쓸 수 없다. 입력이 반사인파를 따라 움직이므로 공칭점이 없고, 탱크 하나로 게인 범위를 덮지 못하므로 '
    ...
    add(p('그림&nbsp;%s 에서 라인 반주기 하나를 따라 제어 법칙을 따라간다. 앞의 세 '
    ...
    add(note('<b>영교차 dead band.</b> &theta;&nbsp;= 0 바로 근처에서 프로파일은 '
             'f<sub>Min</sub> 아래의 주파수를 요구하고, 컨버터는 상용전원 전압이 다시 올라올 때까지 전류를 끌지 않는다. 이것은 구조에 내재한 것이고 '
    ...
    add(note('<b>이 차트에서 흔히 하는 실수:</b> 가장 높은 곡선을 가장 낮은 선과 '
             ...
             '함께 정하므로 같은 색끼리 비교해야 뜻이 있다. 이 문서가 여러 입력 전압을 한 그림에 겹쳐 그리지 않는 이유도 같다: 라인 '
    ...
    add(p('버스트 모드 식과 블록도의 multiplier 를 합치면 피드백 전압의 의미가 나온다.'))
    ...
    add(note('해가 없는 것이 실패는 아니다: 높은 입력 전압의 No load 는 '
             ...
             '주파수만으로 조절할 수 있는지를 이것이 판별하고, Full load 숫자로는 '
             '보이지 않는다.'))
    ...
    add(fig('f13_morphing',
            '모핑이 2.93:1 의 상용전원 범위를 탱크에서 1.92:1 로 줄인다. 최악 '
    ...
    add(p('올라갈 때는 245&nbsp;V<sub>pk</sub> 에서 하프브리지로 떨어지고, '
          ...
          '히스테리시스가 잦은 모드 전환은 막지만, <b>둘 사이에서는 모드가 전압이 아니라 '
          '지나온 방향에 달렸다</b>.'))
    ...
    add(p('P<sub>in,LLC</sub> 는 공진단으로 들어가는 전력이다. LLC 교과서는 '
          '전부 8/&pi;&sup2; 과 2-stage 컨버터의 출력 전력을 쓴다. 여기서 '
    ...
    ext(tbl('그림의 각 표시가 무엇을 정하나. 전류는 유닛 하나의 값이다: '
            ...
            [['표시', '항목', '어디서 &mdash; 무엇에 걸쳐 읽나',
              '무엇을 정하나'],
             ['<b>1</b>', '1차 피크 전류',
              'i<sub>p</sub> 의 피크: 자화 램프에 부하 전류의 반사인파를 더한 것. 최악 스위칭 사이클: 라인 피크, 최저 등가 입력, '
              'Full load.',
    ...
    add(p('&lambda;&nbsp;&asymp;&nbsp;0.5 에서 <b>약 1/3</b> 이다. 보통의 트랜스포머는 누설이 몇 % 인데, 이것은 일부러 그 열 배를 누설시켜야 한다. 그만한 누설은 권선 <b>배치</b>로 만들고, 그 배치는 창 면적을 차지한다. '
    ...
    ext(bullets([
        ...
        '근처, 폭이 전류를 감당한다. 센터탭에 단자를 내기도 좋다.',
    ...
    add(p('<b>사양서의 그 줄이 말하는 것.</b> 트랜스포머 사양서에는 "DC '
          'overlap: 초기 인덕턴스의 90&nbsp;% 이상, 시험 전류 I<sub>sat</sub>, '
          '상온" 꼴의 줄이 있다. 이름이 곧 측정 방법이다. LCR 미터가 작은 AC '
    ...
    add(p('<b>이 시험이 드러내는 것.</b> 다른 권선이 open 이면 DC 전류 하나가 '
          ...
          '다가가면 떨어진다. 90&nbsp;% 점이 그 무릎이다. 단자에서 자속 '
    ...
    add(p('<b>그 줄에 어느 전류를 적나.</b> 그림&nbsp;%(f)s 의 표시 <b>3</b> 과 '
    ...
    add(p('<b>상한이 OVP2 인 이유.</b> 자속은 출력 전압만 따라가므로(%s 절), '
          '코어가 평생 겪는 가장 큰 자속은 컨트롤러가 허용하는 가장 높은 '
    ...
    add(eq(r'I_{sat}\;=\;I_{eq}\;\frac{V_{OVP2}}{V_{out,eff}}',
           key='Isatspec'))
    add(p('자속을 올리지 않는 것: Overload(동선과 반도체가 받는다), '
          '기동(출력이 더 낮다), 버스트 모드, 스위칭 주파수(f<sub>sw</sub> 가 '
          '무엇이든 클램프는 T<sub>r</sub>/2 동안이다). 올리는 것: 출력 과전압, '
          '그리고 온도. MnZn 파워 페라이트의 B<sub>s</sub> 는 25 &deg;C 에서 '
    ...
    ext(bullets([
        ...
        '%s 절에 이 설계의 비가 있다.' % SR('트랜스포머, 만든 대로'),
        '<b>관행값</b>, 즉 상온에서 i<sub>&mu;,pk</sub> 나 권선 피크의 1.2 ~ '
        '1.5 배는 아무도 이 질문을 하지 않았을 때 사양서에 실리는 숫자다. '
        '어떤 사건도 가리키지 않으므로 안전하지도 경제적이지도 않다. 위 '
        '셋 중 하나로 바꾼다.']))
    ...
    add(p('뱅크를 정하기 전에 묻는 것이므로 <i>허용</i> &Delta;v 를 쓴다. '
    ...
    ext(tbl('루프가 이뤄야 할 것.',
            [['항목', '목표', '너무 낮으면', '너무 높으면'],
             ...
             ['phase margin &Phi;<sub>M</sub>',
              ...
              '응답이 과감쇠로 느리다. 고장은 아니지만 공짜도 아니다: 그러려면 crossover 주파수를 내려야 한다.'],
             ['gain margin GM',
              '6 dB 가 하한, 10 dB 면 여유 있다.',
              '부품 산포가 |T| 를 올린다. 옵토커플러 CTR 만으로도 랭크·전류·온도에 '
    ...
    add(p('그림&nbsp;%(f)s 에 세 숫자를 어디서 읽는지 보였다. 주파수를 루프의 crossover 주파수로 정규화했으므로 이런 루프 전부에 같은 모양이다. %(s)s 절이 이 '
          '설계의 숫자를 같은 그림에 올린다.'
    ...
    add(p('k&nbsp;=&nbsp;1.05 는 5 % 의 여지다. 가장 작은 k 가 설계의 가장 약한 곳이다.'))
    ...
    add(fig('an_gain_design',
            ...
            '탱크에는 비슷해 보인다. 위상마다 같은 색의 점선과만 비교한다.'
    ...
    add(p('<b>이 설계의 숫자로 본 OVP2 의 이유.</b> OVP1 은 %(o1).2f&nbsp;V 로 '
          ...
          '%(b2).0f&nbsp;mT 가 이 코어에 요구되는 가장 큰 자속이다. 재료 '
          '데이터의 %(mat)s 고온 B<sub>s</sub> 와 견줄 숫자가 이것이다. '
          '사양서의 줄은 이렇다: DC overlap, 초기 인덕턴스의 90&nbsp;%% 이상, '
    ...
    add(p('<b>이 부품에서 시험하는 법.</b> LCR 미터를 1차(NP1, 표&nbsp;%(t)s 의 '
          ...
          '더 일찍 포화한다. 곡선의 무릎이 %(isat).0f&nbsp;A 아래에 있는 부품은 '
    ...
    add(p('<b>OVP2 가 없었다면.</b> 전류 제한이 유일한 상한이 된다: OCP1 은 '
          ...
          '%(rule).1f&nbsp;A 로, 우연히 OVP2 값과 비슷할 뿐 뒤에 아무 사건도 '
          '없다.'
    ...
    add(note('유니버설 입력이면 창은 15&nbsp;k&Omega; ~ %(max).1f&nbsp;k&Omega; '
             '이고, 모핑 한계 47&nbsp;k&Omega; 에는 이를 일이 없다. LOUT2 에는 '
    ...
    add(p('순서대로. 앞의 넷이 설계가 온전한지를 판별한다.'))
    ext(bullets([
