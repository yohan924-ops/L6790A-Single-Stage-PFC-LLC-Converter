...
def legal():
    ...
    for t in (
        ...
        '데이터시트로 다시 확인할 것.',
        '본문의 설계 예제는 아직 시제품으로 검증하지 않았다. 실측이라고 적은 값만 '
        '실측이고 나머지는 계산값이다.',
        '그림은 전부 이 문서를 위해 새로 그렸다. 다른 회사 Application Note 의 '
        '설명을 따른 그림은 캡션에 출처를 적었다.',
    ):
        ...
        [[Paragraph(c, A.S['th']) for c in ('버전', '날짜', '내용')],
         ...
         [Paragraph('V1.3', A.S['tc']), Paragraph('2026년 9월', A.S['tc']),
          Paragraph(A.T('설계 예제를 트랜스포머 한 개(ETD 49/25/16DG)로 하고 '
                        '권선과 핀 배정을 그렸다. 보상 회로 장을 루프 수식, TL431 '
                        '회로의 OPAMP 등가, 루프 설계 예제와 함께 다시 썼다. '
                        '본문 전체를 간결하게 다시 썼고, 결과를 입력 전압 여섯 조건에서 제시한다. '
                        '영문판 V1.3 과 같은 내용.'),
                    A.S['tc'])]],
    ...
    s.extend(A.bullets([
        ...
        '<b>&lambda;</b> = L<sub>r</sub>/L<sub>m</sub> — 누설 인덕턴스 대 자화 인덕턴스의 비. '
