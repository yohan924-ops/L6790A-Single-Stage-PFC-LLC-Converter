...
ACCEPTED = {
    ...
    'k.auxr': 'ST 툴의 **직결** 판정(보조권선을 VCC 에 바로 물릴 때 OVP2 에서 25 V 이하인가)이다. '
              '본 설계는 2026-09-25 부터 보조권선 2T 가 **제너 레귤레이터를 거쳐** VCC 를 대므로 '
              '25 V 한계는 레귤레이터 출력에 걸리고 그것은 `k.VCC` 가 판정한다(시트 §14c). '
              '워크북과 값을 맞추려고 계산은 남겨 둔다 — 워크북과 **의도된 차이**',
}
